...
export const SwipeEventCard: FC<CardProps> = ({ event, className }) => {
  ...
  const truncatedLocation =
    event?.location?.length > 38
      ? event.location.substring(0, 38) + "..."
      : event.location;
      ...
      >
        ...
        <div className="p-4 flex flex-col">
          <Link
            ...
            className="font-bold text-lg text-gray-950 hover:underline min-h-16"
          >
            {event.name}
          </Link>
          ...
          <p className="text-sm text-gray-800 flex items-center gap-2">
            <CurrencyEuroIcon className="!h-4 !w-4 text-gray-700" />€
            {event.price}
          </p>

          {event.description && event.description.length > 0 && (
            <p className="text-gray-500 mt-2 overflow-y-hidden">
...
export const SwipeEventCard2: FC<CardProps> = () => {
  return <div className="w-full h-96"></div>;
};
